...
head(Sales_data)
dim(Sales_data) #Dataset dimensions
str(Sales_data) #Dataset structure
#a).
#To find the number of pencils, we first create a sub-dataset from copy of sales
Pencils <- Sales_data[Sales_data$Item == "Pencil",]
View(Pencils)#creating a table
total_Pencils <- sum(Pencils$Units)
View(total_Pencils)
...
S + geom_col(fill = "skyblue") + labs(title = "REP SALES")
...
Region.ggplot + geom_col(fill = "purple" ) + labs(title = "REGION SALES")
...
clean$profit <- ifelse(clean$Total > d, "Poor Quality", "Good Quality")
View(clean)
str(clean)
