...
# initiates ggplot
library(ggplot2)
otim<- ggplot(women,aes(height, weight))
head(women)
otim + geom_point(alpha = 1/3) + facet_wrap # adds points
otim + facet_wrap(bmicat ~ no2dec, nrow = 2, ncol = 4) # make panels
otim + geom_smooth(method = "lm", se = F, col = "steelblue")# adds a smoother
otim + theme_bw(base_family = "Avenir", base_size = 10) # change theme
otim + labs(x = "Height in meteres")
otim + labs(y = "Weight in kg")
otim + labs(title = " Women in USA")
